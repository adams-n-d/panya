...
int csvOut(vector<Point>, vector<Point>, vector<Point>, vector<double>, string);
int writeComposite(vector<Point>, vector<Point>, vector<Point>, string, Size);
string getNewExt(string filename, string ext);


/*reads a video file, marks the pixel coordinates and writes to file*/
int r, g, b;
int vidAnalyze(const string filename, int* colors){
    int v;
    int e = 0;
 ...
 */
 ...
 //write video with better tracking
    ...
    if(v){
        ...
        string outfile = getNewExt(filename, "out.avi").c_str();
...
int csvOut(vector<Point> Locb, vector<Point> Locg, vector<Point> Locr, vector<double> dir, string csvfilename){
    //rename file to .csv
    string str_name = getNewExt(csvfilename, "csv");
    const char * csvfile = str_name.c_str();
...
int writeComposite(vector<Point> Locb, vector<Point> Locg, vector<Point> Locr, string jpgfilename, Size s){
    ...
    //rename file to .csv
    string jpgfile = getNewExt(jpgfilename, "jpg");
...
string getNewExt(string filename, string ext){
    ...
    filename.replace(loc+1, extLen, ext);
